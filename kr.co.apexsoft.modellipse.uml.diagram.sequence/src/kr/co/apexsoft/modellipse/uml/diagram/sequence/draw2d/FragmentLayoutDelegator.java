...
import kr.co.apexsoft.modellipse.uml.diagram.sequence.util.ConstraintEntry;
import kr.co.apexsoft.modellipse.uml.diagram.sequence.util.ConstraintRegistry;
import kr.co.apexsoft.modellipse.uml.diagram.sequence.util.InteractionOperandCoveredHelper;
...
public class FragmentLayoutDelegator {
	...
	private final static int EXEC_HEAD_HEIGHT_BEFORE_EXEC = 50;
	...
	private void calculateBounds(IFigure parent) {
		...
		ConstraintRegistry.INSTANCE.clear();
		...
		for (int i = 0; i < orderingFragments.size(); i++) {
			...
			if (fragment instanceof CombinedFragment) {
				...
				if (!fConstraintStack.isEmpty()) {
					...
					if (entry.getKey() instanceof ExecutionSpecification) {
						offsetY = entry.getValue().y + EXEC_HEAD_HEIGHT_BEFORE_CF;
					} else {
			...
			// ES
				...
				if (!fConstraintStack.isEmpty()) {
					ConstraintEntry entry = fConstraintStack.peek();
					if (entry.getKey() instanceof ExecutionSpecification) {
						offsetY = entry.getValue().y + EXEC_HEAD_HEIGHT_BEFORE_EXEC;
					} else {
						offsetY = entry.getValue().bottom() + SPACING;
					}
				}
			...
			// EOS
				...
				Stack<ConstraintEntry> tempStack = new Stack<ConstraintEntry>();
				while (!fConstraintStack.isEmpty()) {
					ConstraintEntry entry = fConstraintStack.pop();
					if (es == entry.getKey()) {
				...
				while (!tempStack.isEmpty()) {
					fConstraintStack.push(tempStack.pop());
				}
			}
		}
	}
